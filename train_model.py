...
def train_and_save_model(data_filepath, vectorizer_path, matrix_path, df_path):
    ...
    # We now analyze character n-grams within word boundaries (analyzer='char_wb').
    # This makes the search extremely resilient to spelling errors.
    # ngram_range=(2, 5) looks at letter chunks of size 2, 3, 4, and 5.
    tfidf = TfidfVectorizer(
        analyzer='char_wb',  # Analyze character n-grams within word boundaries
        ngram_range=(2, 5),   # Use n-grams of lengths 2 through 5
        min_df=2             # Ignore n-grams that appear in only one document (reduces noise)
    )
    # --- END OF CHANGE ---
...
if __name__ == '__main__':
    # --- UPDATED: Path to be relative to the script's location ---
